...
var FLFxChannelCount = 125; //64 for old version
...
var FLP_Text = FLP_Undef; //+Size (var length)+Text
 // (Null Term. String)
var FLP_Text_ChanName = 203 // name for the current channel || change to 192 for older version
...
states[STATE_EVENT] = function(parser) {
  ...
  if (eventId >= FLP_Text) {
    ...
    text = parser.readString(textLen) // CHECK ENCODING OF THIS FUNCTION
  ...
  case FLP_Text_SampleFileName:
    if (cc) {
      cc.sampleFileName = text.replace(/\s+/g, '');
      cc.generatorName = "Sampler";
      parser.project.samplesList.push(cc.sampleFileName);
    }
    break;
  ...
  case FLP_Text_PluginName:
    ...
    //if (pluginName) parser.project.effectPlugins.push(pluginName);
    if (parser.debug) {
  ...
  case FLP_Text_BasicChanParams:
    //console.log(intList, parser.project.versionSpecificFactor)
...
function finishParsing(parser) {
  ...
  for (i = 0; i < parser.project.channels.length; i += 1) {
    ...
    if(parser.project.channels[i].generatorName != '' && parser.project.channels[i].generatorName != 'Sampler') 
      parser.project.effectPlugins.push(parser.project.channels[i].generatorName) 
  }
...
function cmpProjectVersions(ver1,ver2){
  if (!ver1 || !ver2) return "[Error] Types don't match"
  for (var i = 0, c='',v=''; c=ver1.charAt(i), v=ver2.charAt(i); i++) {
      if (isNaN(parseInt(c)) || isNaN(parseInt(c)) || parseInt(c)==parseInt(v)) continue
      return c > v ? 1 : -1
  }
  return 0
}
...
FlpParser.prototype.readString = function(len) {
  ...
  if(cmpProjectVersions(this.project.versionString,'20.0.0.0') >= 0){
    this.cursor += len;
    return this.strbuf.toString('utf16le')
  }
